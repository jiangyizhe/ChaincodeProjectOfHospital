...
func AddPatient(patient PatientInfo)(err error){
	err= db.Create(&patient).Error

	return
}
func SelectPatient()(patient []PatientInfo,err error){
	err = db.Debug().Model(&PatientInfo{}).Find(&patient).Error
	return
}

func DelPatient()(patient []PatientInfo,err error){
	err = db.Debug().Model(&PatientInfo{}).Delete(&patient).Error
	return
}