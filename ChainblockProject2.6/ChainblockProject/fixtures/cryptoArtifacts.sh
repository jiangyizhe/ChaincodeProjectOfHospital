...
## Replace example to  in coypto-config.yaml & configtx.yaml
function replaceName() {
    ...
    sed $OPTS "s/example/shuibian.hospital/g" crypto-config.yaml
    sed $OPTS "s/example/shuibian.hospital/g" configtx.yaml
}
...
function generateChannelArtifacts() {
    ...
    $CONFIGTXGEN -profile OneOrgChannel -outputAnchorPeersUpdate ./artifacts/Org1MSPanchors.tx -channelID $CHANNEL_NAME -asOrg HospitalOrg
